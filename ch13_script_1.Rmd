...
ggplot(nes, aes(x = income, y = dvote)) +
  geom_jitter(alpha = .5, height = .4) +
  geom_smooth(method = "lm")

# linear model not appropriate because no possibility of anything greater than
...
# if you are a male, then there is a decrease of .04 probability of voting
# democrat when income is held constant for every increase in income, there is a
# .07 probability decrease of voting democrat when gender is held constant
# Dear non-mathy boss, if we control for gender for every increase in income category, there is a .07 decrease in probability that a person will vote democrat. 
...
nes %>% 
  ...
  summarize(perc.dem.vote = mean(dvote)) %>% 
  mutate(emp.logit = qlogis(perc.dem.vote)) %>% 
  ggplot(aes(x = year, y = emp.logit)) + geom_point() + facet_wrap(~gender) +
  ylim(-1, 0.5) + labs(title = "Trends favoring the Democrats for both Male and Female",
                       y = "Empirical logits", x = "Year", 
